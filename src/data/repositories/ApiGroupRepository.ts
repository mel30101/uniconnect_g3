...
export class ApiGroupRepository implements IGroupRepository {
  async getUserGroups(userId: string, role: 'admin' | 'student'): Promise<Group[]> {
    const res = await apiClient.get(`/api/groups/user/${userId}?role=${role}`);
    return res.data;
  }
  ...
  async createGroup(name: string, subjectId: string, description: string, creatorId: string): Promise<any> {
    const res = await apiClient.post('/api/groups', { name, subjectId, description, creatorId });
    return res.data;
  }

  async sendJoinRequest(groupId: string, userId: string, userName: string): Promise<any> {
    const res = await apiClient.post(`/api/groups/${groupId}/requests`, { userId, userName });
    return res.data;
  }

  async getRequests(groupId: string): Promise<any[]> {
    const res = await apiClient.get(`/api/groups/${groupId}/requests`);
    return res.data;
  }
  ...
  async removeMember(groupId: string, userId: string, adminId: string): Promise<any> {
    const res = await apiClient.delete(`/api/groups/${groupId}/members/${userId}?adminId=${adminId}`);
    return res.data;
  }

  async transferAdmin(groupId: string, adminId: string, newAdminId: string): Promise<any> {
    const res = await apiClient.put(`/api/groups/${groupId}/transfer-admin`, { adminId, newAdminId });
    return res.data;
  }
  ...
  async getAvailableStudents(groupId: string, subjectId: string, search: string = ''): Promise<any[]> {
    const res = await apiClient.get(`/api/groups/${groupId}/available-students?subjectId=${subjectId}&search=${search}`);
    return res.data;
  }
}
